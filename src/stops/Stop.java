...
public class Stop extends Object
     ...
     * after strings (e.g. {name}), the spaces should be considered part of the
     ...
     * stop name (as this is defined as invalid according to the Stop
     * constructor).
     */
    public static Stop decode(String stopString)
        ...
        String stopName;
        try {
            stopName = stopString.split(":")[0];
            xCoordinate = Integer.parseInt(
                    stopString.split(":")[1].trim());
            yCoordinate = Integer.parseInt(
                    stopString.split(":")[2].trim());

        } catch (Exception e) {
            throw new TransportFormatException();
        }

        return new Stop(stopName, xCoordinate, yCoordinate);
